...
def parse_prompt_regex(prompt, system_names=None, systems=None):
    ...
    if not preset:
        # Precompute every element used by ANY known preset, so we can
        # tell "chromium" is a legitimate alias hit vs. one word in a
        # combination (e.g. "iron chromium") that's actually unrecognized
        # -- don't let a single alias keyword hijack the match when
        # another clearly-mentioned element belongs to no known system.
        all_system_elements = set()
        if systems:
            for sys_name in system_names:
                comps = systems[sys_name]["config"].get("COMPONENTS", [])
                if isinstance(comps, str):
                    comps = [comps]
                all_system_elements.update(str(c).upper() for c in comps)
        ...
        for name, aliases in SYSTEM_NAME_ALIASES.items():
            ...
            if not any(
                ...
                continue
            alias_words_lower = {a.lower() for a in aliases}
            unrecognized_elsewhere = {
                sym.upper() for word, sym in ELEMENT_KEYWORDS.items()
                if word.lower() not in alias_words_lower
                and re.search(rf"\b{word}\b", prompt, re.IGNORECASE)
                and sym.upper() not in all_system_elements
            }
            if unrecognized_elsewhere:
                continue  # e.g. "iron" (Fe) isn't used by any preset -- don't force-match
            preset = name
            break
