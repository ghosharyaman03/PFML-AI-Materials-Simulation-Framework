#ifndef FUNCTION_F_06_H_
#define FUNCTION_F_06_H_
...
double function_F_06_free_energy(double *c, double T, long a) {
  long i,j;
  double sum=0.0;
...
void function_F_06_Mu(double *c, double T, long a, double *Mu) {
  long j,k;
  double sum=0.0;
...
void function_F_06_c_mu(double *mu, double *c, double T, long a, double *c_guess) {
  long k,l;
  double sum=0.0;
//   if(!ISOTHERMAL) {
//     for(i=0; i<NUMCOMPONENTS-1; i++) {
...
void function_F_06_dc_dmu(double *mu, double *phase_comp, double T, long a, double **dcdmu) {
  long i, j;
//   if(!ISOTHERMAL) {
//     if (a!=(NUMPHASES-1)) {
//       for(i=0; i<NUMCOMPONENTS-1; i++) {
...
double function_F_06_dpsi(double *mu, double **phase_comp, double T, double *phi, long a) {
  double psi=0.0;
...
void function_F_06_function_A(double T, double ***c) {
  long i,j,k, index;
  double **dmudc_matrix;
  double sum=0.0;
...
double function_F_06_function_B(double T, long i, long a) {
...
double function_F_06_function_C(double T, long a) {
  double c_liq[NUMCOMPONENTS-1];
  double c_sol[NUMCOMPONENTS-1];
...
void function_F_06_init_propertymatrices(double T) {
...
//         comp_ES[a][k][lines] = composition_liquid[k] - composition_solid[k];
  ...
  double T_solidus  = T_ES[0][0];
  double T_liquidus = T_ES[0][numlines-2];
