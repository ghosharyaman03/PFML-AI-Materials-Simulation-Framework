  ...
  for(x=0; x < workers_mpi.rows_x; x++) {
   for(z=0; z < workers_mpi.rows_z; z++) {
    for(y=0; y < workers_mpi.rows_y; y++) {
      ...
      gridinfo_w[gidy].temperature -= GRADIENT*(temperature_gradientY.velocity/deltay)*deltat;
...
void fill_temperature_gradientY(struct fields* gridinfo_w, long shift_OFFSET, long t) {
  long x, y, z, endx;
  long gidy;
  double BASE_POS=0;
  double GRADIENT;
