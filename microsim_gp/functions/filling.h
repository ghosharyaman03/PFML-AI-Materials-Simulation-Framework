...
/*
 * ============================================================================
 * Domain Initialization and Geometry Filling Utilities
 * ============================================================================
 *
 * Description:
 * ------------
 * This file contains a collection of functions used to initialize and populate
 * a 2D/3D simulation domain with different geometric structures, phase fields,
 * and physical properties (e.g., velocity, composition).
 *
 * The domain is discretized into a structured grid of size:
 *      rows_x × rows_y × rows_z
 *
 * Each grid point stores:
 *  - Phase fractions (phia[])
 *  - Composition (composition[])
 *  - Chemical potentials (compi[])
 *  - Velocity fields (for LBM simulations)
 *
 * Core Responsibilities:
 * ----------------------
 * 1. Geometry Construction:
 *    - Cubes, spheres, cylinders, ellipses
 *    - Structured patterns and randomized distributions
 *    - Voronoi tessellations (2D and 3D)
 *
 * 2. Phase Assignment:
 *    - Assigns phase fractions (phia[]) to grid points
 *    - Ensures phase consistency:
 *          sum(phia) = 1.0
 *    - The last phase (NUMPHASES - 1) is typically treated as the matrix phase
 *
 * 3. Random Microstructure Generation:
 *    - Random placement of particles (cubes, spheres, cylinders)
 *    - Volume fraction control
 *    - Size variability using spread parameters
 *    - Overlap prevention using:
 *          - checkOverlap() (distance-based)
 *          - check_overlap_sq() (bounding-box based)
 *
 * 4. Occupancy Tracking:
 *    - Uses auxiliary occupancy arrays to prevent particle overlap
 *    - Marks grid cells already occupied by particles
 *
 * 5. Velocity Initialization (LBM-specific):
 *    - fill_velocity_cube() assigns constant velocity vectors (ux, uy, uz)
 *      within a specified region
 *    - Density (rho) is initialized to 1.0 everywhere
 *
 * 6. Composition Initialization:
 *    - fill_composition_cube() assigns equilibrium compositions based on phase
 *    - Uses thermodynamic functions (e.g., Mu())
 *    - Falls back to matrix composition if no solid phase is present
 *
 * Key Functions:
 * --------------
 * Geometry Filling:
 ...
 *
 * Randomized Filling:
 *  - fill_phase_sphere_random()
 *  - fill_phase_cylinder_random()
 *  - fill_phase_cube_random_variants()
 *
 * Pattern-Based Filling:
 *  - fill_cube_pattern()
 *
 * Voronoi Structures:
 *  - fill_phase_voronoi_2D()
 *  - fill_phase_voronoi_3D()
 *
 * Overlap Checking:
 *  - checkOverlap()         : distance-based overlap detection
 *  - check_overlap_sq()     : bounding-box overlap detection
 *
 * Velocity Initialization:
 *  - fill_velocity_cube()
 *
 * Composition Initialization:
 *  - fill_composition_cube()
 *
 * Important Notes:
 * ----------------
 * - The last phase (NUMPHASES - 1) is treated as the background/matrix phase.
 * - Many routines ensure phase normalization by computing:
 *        phia[last] = 1.0 - sum(other phases)
 * - Random generators:
 *      - drand48() (standard)
 *      - GSL RNG (for more controlled randomness)
 *
 * - Boundary checks are applied to ensure particles remain within domain limits.
 *
 * - Random placement loops include a safeguard (random_limit) to avoid infinite
 *   loops when the domain becomes too crowded.
 *
 * Dependencies:
 * -------------
 * - Global variables:
 *      rows_x, rows_y, rows_z, layer_size
 *      NUMPHASES, NUMCOMPONENTS
 *      MESH_X, MESH_Y, MESH_Z
 *
 * - External structures:
 *      struct fields
 *      struct fill_cube / fill_sphere / fill_cylinder / fill_ellipse
 *      struct lbm_fields
 *
 * - External functions:
 *      Mu(), gsl_rng_*(), etc.
 ...
 *      occupancy arrays
 *      Voronoi temporary arrays
 * - All allocated memory is freed after use
 *
 * ============================================================================
 */
...
void fill_velocity_cube(struct fill_cube fill_cube_parameters, struct lbm_fields *lbm_gridinfo, double ux, double uy, double uz) {
  long x, y, z, index;
  for(x=0;x < rows_x; x++) {
    for(z=0; z < rows_z; z++) {
      for (y=0; y < rows_y; y++) {
        ...
        if ((x >= fill_cube_parameters.x_start) && (x <= fill_cube_parameters.x_end) && (y >=fill_cube_parameters.y_start) && (y<=fill_cube_parameters.y_end) && (z >=fill_cube_parameters.z_start) && (z<=fill_cube_parameters.z_end)) {
          lbm_gridinfo[index].u[X] = ux;
          lbm_gridinfo[index].u[Y] = uy;
          lbm_gridinfo[index].u[Z] = uz;
         }
         lbm_gridinfo[index].rho = 1.0;
      }
    }
  }
}
...
void fill_phase_cylinder_notexcluding_last_phase(struct fill_cylinder fill_cylinder_parameters, struct fields* gridinfo, long b) {
  long x, y, z, index;
  long a;
  double sum;
      ...
      for (y=0; y < rows_y; y++) {
        index = x*layer_size + z*rows_y + y;
          ...
          if (sum > 1.0) {
            ...
            gridinfo[index].phia[NUMPHASES-1] = 1.0-sum;
          }
      }
    }
  }
...
void fill_phase_sphere_notexcluding_last_phase(struct fill_sphere fill_sphere_parameters, struct fields* gridinfo, long b) {
  long x, y, z, index;
  long a;
  double sum;
...
void fill_phase_sphere_occupancy_notexcluding_last_phase(struct fill_sphere fill_sphere_parameters, struct fields* gridinfo, long b, int* occupancy) {
  long x, y, z, index;
  long a;
  double sum;
...
void fill_phase_sphere_random_notexcluding_last_phase(long phase, double ppt_radius, double volume_fraction, long shield_dist, double spread) {
  int overlap = 1;
  int *occupancy;
  long i;
  occupancy = (int*)malloc(sizeof(int)*rows_x*rows_y*rows_z);
  ...
  while (particle_index <= num_particles) {
      ...
      if(overlap == 0) {
        fill_phase_sphere_occupancy_notexcluding_last_phase(temp_sph, gridinfo, phase, occupancy);
        //fill_phase_cylinder(temp_cyl, gridinfo, b);
      }
      else {
        overlap = 1;
...
void fill_phase_cylinder_occupancy_notexcluding_last_phase(struct fill_cylinder fill_cylinder_parameters, struct fields* gridinfo, long b, int* occupancy) {
    long x, y, z, index;
    long a;
    double sum;
...
void fill_phase_cylinder_random_notexcluding_last_phase(long phase, double ppt_radius, double volume_fraction, long shield_dist, double spread) {
    int overlap = 1;
    int *occupancy;
    long i;
    occupancy = (int*)malloc(sizeof(int)*rows_x*rows_y*rows_z);
...
//         printf("particle_index = %d, x_center = %ld, y_center = %ld, z_start = %ld, z_end = %ld, radius = %lf\n", particle_index, centx, centy, temp_cyl.z_start, temp_cyl.z_end, rad);
        ...
        if(overlap == 0)
        {
            fill_phase_cylinder_occupancy_notexcluding_last_phase(temp_cyl, gridinfo, phase, occupancy);
//             fill_phase_cylinder_occupancy(temp_cyl, gridinfo, 0, occupancy);
//             fill_phase_cylinder(temp_cyl, gridinfo, b);
        }
        else
