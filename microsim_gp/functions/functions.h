...
void allocate_memory_gradlayer(struct gradlayer *ptr);
...
void Build_derived_type_lbm(MPI_Datatype* MPI_lbm_gridinfo) {
  MPI_Type_contiguous(SIZE_LBM_FIELDS, MPI_DOUBLE, MPI_lbm_gridinfo);
  MPI_Type_commit(MPI_lbm_gridinfo);
}
// void Build_derived_type_lbm_3D(MPI_Datatype* MPI_lbm_gridinfo) {
//   MPI_Type_contiguous(SIZE_LBM_FIELDS, MPI_DOUBLE, MPI_lbm_gridinfo);
//   MPI_Type_commit(MPI_lbm_gridinfo);
// }
...
void EIDT_Update_FFComp(long step)
{
  ...
  time   = (double)(step - EIDT.step0);
  time   = time < 0 ? 0 : time;
  theta  = sqrt(2) * sqrt( -1.0 + sqrt( 1.1 +  pow(EIDT.step_coeff,2)*pow(time+1,2)));
  radius = EIDT.radius_coeff*theta;  

  EIDT.comp_far_field[0] = time <= 0.0 ? EIDT.comp_center[0] : EIDT.comp_center[0] - radius*cos(theta -(3.14152857/6.0));
  EIDT.comp_far_field[1] = time <= 0.0 ? EIDT.comp_center[1] : EIDT.comp_center[1] + radius*sin(theta);
}
