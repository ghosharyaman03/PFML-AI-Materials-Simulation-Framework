...
void serialinfo_xy() {
  ...
  if (LBM) {
    lbm_gridinfo = (struct lbm_fields* )malloc((index_count*(SIZE_LBM_FIELDS))*sizeof(struct lbm_fields));
    for (index=0; index < index_count; index++) {
      allocate_memory_lbm_fields(&lbm_gridinfo[index]);
    }
//     if (DIMENSION == 2) {
//       lbm_gridinfo_2D = (struct lbm_fields_2D* )malloc((index_count)*sizeof(struct lbm_fields_2D));
//       struct lbm_fields_2D *lbm_gridinfo;
//       lbm_gridinfo = lbm_gridinfo_2D; 
// //       SIZE_LBM_FIELDS = 11;
//     }
//     if (DIMENSION == 3) {
//       lbm_gridinfo_3D = (struct lbm_fields_3D* )malloc((index_count)*sizeof(struct lbm_fields_3D));
//       struct lbm_fields_3D *lbm_gridinfo;
//       lbm_gridinfo = lbm_gridinfo_3D;
// //       SIZE_LBM_FIELDS = 31;
//     }
  }
...
//   printf("taskid=%d, size=%ld\n",taskid, sizeof(double));
  ...
  global_max_min.rel_change_phi_max     = (double*)malloc((NUMPHASES)*sizeof(double));
  global_max_min.rel_change_mu_max      = (double*)malloc((NUMCOMPONENTS-1)*sizeof(double));
