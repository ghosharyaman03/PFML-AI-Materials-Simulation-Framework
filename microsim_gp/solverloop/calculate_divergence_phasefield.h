...
void calculate_divergence_phasefield_2D(long x, struct gradlayer **gradient) {
  ...
  //     grad->stiffness_c[Y] = calculate_stiffness(center);
      ...
      sigma.xx  = grad->stiffness_c[X].C11*(avg_strain.xx + grad->strain[X].xx)  + grad->stiffness_c[X].C12*(avg_strain.yy + grad->strain[X].yy);
      sigma.yy  = grad->stiffness_c[X].C12*(avg_strain.xx + grad->strain[X].xx)  + grad->stiffness_c[X].C11*(avg_strain.yy + grad->strain[X].yy);
      sigma.xy  = 2.0*grad->stiffness_c[X].C44*(avg_strain.xy + grad->strain[X].xy);
    }
...
//         for (b=0; b < NUMPHASES; b++) {
//           if (b < a) {
//             sum_qab += (gridinfo_w[center].phia[a]*grad->gradphi_c[X][b] - gridinfo_w[center].phia[b]*grad->gradphi_c[X][a])*(gridinfo_w[center].phia[a]*grad->gradphi_c[X][b] - gridinfo_w[center].phia[b]*grad->gradphi_c[X][a]);
//             sum_qab += (gridinfo_w[center].phia[a]*grad->gradphi_c[Y][b] - gridinfo_w[center].phia[b]*grad->gradphi_c[Y][a])*(gridinfo_w[center].phia[a]*grad->gradphi_c[Y][b] - gridinfo_w[center].phia[b]*grad->gradphi_c[Y][a]);
//             sum_wab += (16.0/(M_PI*M_PI))*gridinfo_w[center].phia[a]*gridinfo_w[center].phia[b];
//           }
//         }
//       }
//     }
...
//           lambda_phi[a] += AMP_NOISE_PHASE*((double)(rand())/(double)RAND_MAX)*gridinfo_w[center].phia[a]*(1.0-gridinfo_w[center].phia[a]); 
        ...
        if (CONSTRAINED) {
          lambda_phi[a] -= fabs(lambda*(epsilon*(-dAdphi(gridinfo_w[center].phia, gradient, gidy, a) + divdAdgradphi(gradient, center, gidy, a)) + (1.0/epsilon)*dwdphi(gridinfo_w[center].phia, divphi,gradient, gidy, a)));
        }
...
void calculate_divergence_phasefield_3D(long x, struct gradlayer **gradient) {
  ...
  //     grad->stiffness_c[Y] = calculate_stiffness(center);
      ...
      sigma.xx  = grad->stiffness_c[X].C11*(avg_strain.xx + grad->strain[X].xx) + grad->stiffness_c[X].C12*(avg_strain.yy + avg_strain.zz + grad->strain[X].yy + grad->strain[X].zz);
    
      sigma.yy  = grad->stiffness_c[X].C12*(avg_strain.xx + grad->strain[X].xx  + avg_strain.zz + grad->strain[X].zz) + grad->stiffness_c[X].C11*(avg_strain.yy + grad->strain[X].yy);
     
      sigma.zz  = grad->stiffness_c[X].C12*(avg_strain.xx + grad->strain[X].xx  + avg_strain.yy + grad->strain[X].yy) + grad->stiffness_c[X].C11*(avg_strain.zz + grad->strain[X].zz);
      
      sigma.xy  = 2.0*grad->stiffness_c[X].C44*(avg_strain.xy + grad->strain[X].xy);
      
      sigma.xz  = 2.0*grad->stiffness_c[X].C44*(avg_strain.xz + grad->strain[X].xz);
      
      sigma.yz  = 2.0*grad->stiffness_c[X].C44*(avg_strain.yz + grad->strain[X].yz);
    }
