...
void fill_buffer_x_lbm(long start_j, long x_start, long x_end);
void fill_buffer_y_lbm(long start_j, long y_start, long y_end);
void fill_buffer_z_lbm(long start_j, long z_start, long z_end);

void fill_gridinfo_x_lbm(long start_j, long x_start, long x_end);
void fill_gridinfo_y_lbm(long start_j, long y_start, long y_end);
void fill_gridinfo_z_lbm(long start_j, long z_start, long z_end);
...
//   long averow_y;
  ...
  if (taskid == MASTER) {
    ...
    for (rank=0; rank < (numworkers_x*numworkers_y*numworkers_z); rank++) {
      ...
      if (rank != MASTER) {
        if (rank==1) {
          printf("\n- [%ld] sending rows[%ld,%ld,%ld]",rank, rows[X], rows[Y], rows[Z]);
        }
        dest = rank;
        ...
        MPI_Send(buffer_lbm, 1, MPI_gridinfo_vector_lbm, dest, BEGIN, MPI_COMM_WORLD);
        ...
        if (LBM) {
          lbm_gridinfo_w = (struct lbm_fields* )malloc((index_count)*sizeof(struct lbm_fields));
          for (index=0; index < index_count; index++) {
            allocate_memory_lbm_fields(&lbm_gridinfo_w[index]);
          }
          gradc = Malloc3M(workers_mpi.layer_size, DIMENSION, NUMCOMPONENTS-1);
        }
        int layer;
        ...
        //printf("\n- [%ld] (X,Y,Z) rows[%ld,%ld,%ld] rows_(%ld,%ld,%ld) first(%d,%d,%d) last(%d,%d,%d) offset[%ld,%ld,%ld] offset_(%ld,%ld,%ld)\n",taskid,workers_mpi.rows[X],workers_mpi.rows[Y],workers_mpi.rows[Z],workers_mpi.rows_x, workers_mpi.rows_y, workers_mpi.rows_z, workers_mpi.firstx, workers_mpi.firsty, workers_mpi.firstz,workers_mpi.lastx,  workers_mpi.lasty,  workers_mpi.lastz, workers_mpi.offset[X], workers_mpi.offset[Y],workers_mpi.offset[Z],offset_x, offset_y, offset_z);
        ...
        if(LBM){
          workers_max_min.rho_max = 1 ;
          workers_max_min.rho_min = 1 ;
        }
    ...
    if(LBM) {
      ...
      MPI_Recv(buffer_lbm, 1, MPI_gridinfo_vector_lbm, source, msgtype, MPI_COMM_WORLD, &status);
      
      MPI_Type_free(&MPI_gridinfo_vector_lbm);
    }
    
//     printf("Received data, status=%d, test=%d, workers_mpi.offset_x=%ld, workers_mpi.offset_y=%ld, workers_mpi.rows[X]=%ld,workers_mpi.rows_y=%ld,index_count=%ld,workers_mpi.layer_size=%ld\n",status.MPI_ERROR,taskid, MPI_SUCCESS, workers_mpi.offset_x, workers_mpi.offset_y, workers_mpi.rows[X],workers_mpi.rows_y,index_count,workers_mpi.layer_size);
//     
    ...
    if(RESTART == 0) {
      if(LBM) {
        for (index=0; index < index_count; index++) {
          for (k=0; k<(SIZE_LBM_FIELDS-4); k++) {
            lbm_gridinfo_w[index].fdis[k] = 0.0;
          }
          lbm_gridinfo_w[index].rho = 0.0;
          
          for (dim=0; dim<3; dim++) {
            lbm_gridinfo_w[index].u[dim] = 0.0;
          }
        }
        j=0;
        for (i=0; i < workers_mpi.rows[X]*workers_mpi.rows[Y]*workers_mpi.rows[Z]; i++) {
    ...
    //printf("Received data,  rows_x=%ld, rows_y=%ld, firstx=%d, firsty=%d, firstz=%d, lastx=%d, lasty=%d, lastz=%d, offset[X]=%ld, offset[Y]=%ld, offset[Z]=%ld, offset_x=%ld, offset_y=%ld, offset_z=%ld, workers_mpi.layer_size=%ld\n",
...
void mpiexchange_left_right_lbm(long taskid) {
 if (workers_mpi.rank_x%2) {
    if (workers_mpi.firstx ==0) {
      fill_buffer_x_lbm(3*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3, 5);
      
      MPI_Send(buffer_boundary_x_lbm + 3*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3*workers_mpi.layer_size, MPI_lbm_gridinfo, workers_mpi.left_node, RTAG, MPI_COMM_WORLD);

      source  = workers_mpi.left_node;
      msgtype = LTAG;
      MPI_Recv(buffer_boundary_x_lbm + 0*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3*workers_mpi.layer_size, MPI_lbm_gridinfo, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_x_lbm(0, 0, 2);
    }
    if (workers_mpi.lastx == 0) {
      fill_buffer_x_lbm(6*workers_mpi.layer_size*SIZE_LBM_FIELDS, workers_mpi.end[X]-2, workers_mpi.end[X]);
  
      MPI_Send(buffer_boundary_x_lbm + 6*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3*workers_mpi.layer_size, MPI_lbm_gridinfo, workers_mpi.right_node, LTAG,   MPI_COMM_WORLD);
      
      source  = workers_mpi.right_node;
      msgtype = RTAG;
      
      MPI_Recv(buffer_boundary_x_lbm + 9*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3*workers_mpi.layer_size, MPI_lbm_gridinfo, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_x_lbm(9*workers_mpi.layer_size*SIZE_LBM_FIELDS, workers_mpi.end[X]+1, workers_mpi.end[X]+3);
    }
  } else {
    if (workers_mpi.lastx == 0) {
      source  = workers_mpi.right_node;
...
void mpiexchange_top_bottom_lbm(long taskid) {
 long i;
 if (workers_mpi.rank_y%2) {
    if (workers_mpi.firsty ==0) {
     
      fill_buffer_y_lbm(3*SIZE_LBM_FIELDS, 3, 5);
      
      MPI_Send(buffer_boundary_y_lbm + 3*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_b_lbm, workers_mpi.bottom_node,   TTAG, MPI_COMM_WORLD);
      source = workers_mpi.bottom_node;
      msgtype = BTAG;
    
      MPI_Recv(buffer_boundary_y_lbm + 0*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_b_lbm, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_y_lbm(0, 0, 2);
      
    }
    if (workers_mpi.lasty == 0) {
      
      fill_buffer_y_lbm(6*SIZE_LBM_FIELDS, workers_mpi.end[Y]-2, workers_mpi.end[Y]);
      
      MPI_Send(buffer_boundary_y_lbm + 6*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_b_lbm, workers_mpi.top_node,   BTAG, MPI_COMM_WORLD);
      
      source  = workers_mpi.top_node;
      msgtype = TTAG;
      
      MPI_Recv(buffer_boundary_y_lbm + 9*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_b_lbm, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_y_lbm(9*SIZE_LBM_FIELDS, workers_mpi.end[Y]+1, workers_mpi.end[Y]+3);
    }
  } else {
    if (workers_mpi.lasty == 0) {
...
void mpiexchange_front_back_lbm(long taskid) {
 long i;
 if (workers_mpi.rank_z%2) {
    if (workers_mpi.firstz ==0) {
     
      fill_buffer_z_lbm(3*SIZE_LBM_FIELDS, 3, 5);
      
      MPI_Send(buffer_boundary_z_lbm + 3*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, workers_mpi.back_node,   TTAG, MPI_COMM_WORLD);
      source = workers_mpi.back_node;
      msgtype = BTAG;
    
      MPI_Recv(buffer_boundary_z_lbm + 0*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_z_lbm(0, 0, 2);
      
    }
    if (workers_mpi.lastz == 0) {
      
      fill_buffer_z_lbm(6*SIZE_LBM_FIELDS, workers_mpi.end[Z]-2, workers_mpi.end[Z]);
      
      MPI_Send(buffer_boundary_z_lbm + 6*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, workers_mpi.front_node,   BTAG, MPI_COMM_WORLD);
      
      source  = workers_mpi.front_node;
      msgtype = TTAG;
      
      MPI_Recv(buffer_boundary_z_lbm + 9*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, source, msgtype, MPI_COMM_WORLD, &status);
      
      fill_gridinfo_z_lbm(9*SIZE_LBM_FIELDS, workers_mpi.end[Z]+1, workers_mpi.end[Z]+3);
    }
  } else {
    if (workers_mpi.lastz == 0) {
...
void mpiboundary_left_right_lbm(long taskid) {
  if (workers_mpi.firstx ==1) {
//     printf("rank_x=%d, rank=%ld, first_x=%d, last_x=%d, workers_mpi.left_node=%d, workers_mpi.layer_size=%ld\n", workers_mpi.rank_x, taskid, workers_mpi.firstx, workers_mpi.lastx, workers_mpi.left_node, workers_mpi.layer_size);
    fill_buffer_x_lbm(3*workers_mpi.layer_size*SIZE_LBM_FIELDS, 3, 5);
...
void mpiboundary_top_bottom_lbm(long taskid) {
  long i;
  if (workers_mpi.firsty ==1) {
    fill_buffer_y_lbm(3*SIZE_LBM_FIELDS, 3, 5);
...
void mpiboundary_front_back_lbm(long taskid) {
  long i;
  if (workers_mpi.firstz ==1) {
    fill_buffer_z_lbm(3*SIZE_LBM_FIELDS, 3, 5);
        
    MPI_Send(buffer_boundary_z_lbm + 3*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, workers_mpi.back_node,   FTAG, MPI_COMM_WORLD);
    source = workers_mpi.back_node;
    msgtype = BATAG;
  ...
  if(workers_mpi.lastz ==1) {
    ...
    MPI_Send(buffer_boundary_z_lbm + 6*SIZE_LBM_FIELDS, 1, MPI_gridinfo_vector_c_lbm, workers_mpi.front_node, BATAG, MPI_COMM_WORLD);
  }
}
void fill_buffer_x(long start_j, long x_start, long x_end) {
  long j, x, y, z;
  ...
  for (x=x_start; x <= x_end; x++) {
    for (z=0; z < workers_mpi.rows_z; z++) {
      for (y=0; y < workers_mpi.rows_y; y++) {
        index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
        for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
          buffer_boundary_x_lbm[j] = lbm_gridinfo_w[index].fdis[k];
          j++;
        }
        buffer_boundary_x_lbm[j] = lbm_gridinfo_w[index].rho;
        j++;
        for (dim=0; dim<3; dim++) {
          buffer_boundary_x_lbm[j] = lbm_gridinfo_w[index].u[dim];
          j++;
        }
      }
    }
  ...
  for (x=0; x < workers_mpi.rows_x; x++) {
   for (z=0; z < workers_mpi.rows_z; z++) {
    for (y=y_start; y <= y_end; y++) {
      index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
        for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
          buffer_boundary_y_lbm[j] = lbm_gridinfo_w[index].fdis[k];
          j++;
        }
        buffer_boundary_y_lbm[j] = lbm_gridinfo_w[index].rho;
        j++;
        for (dim=0; dim<3; dim++) {
          buffer_boundary_y_lbm[j] = lbm_gridinfo_w[index].u[dim];
          j++;
        }
      }
      j += 9*SIZE_LBM_FIELDS;
  ...
  for (x=0; x < workers_mpi.rows_x; x++) {
   for (y=0; y < workers_mpi.rows_y; y++) {
     for (z=z_start; z <= z_end; z++) {
      index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
       for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
         buffer_boundary_z_lbm[j] = lbm_gridinfo_w[index].fdis[k];
         j++;
       }
        buffer_boundary_z_lbm[j] = lbm_gridinfo_w[index].rho;
        j++;
        for (dim=0; dim<3; dim++) {
          buffer_boundary_z_lbm[j] = lbm_gridinfo_w[index].u[dim];
          j++;
        }
     }
    j += 9*SIZE_LBM_FIELDS;
  ...
  for (x=x_start; x <= x_end; x++) {
    for (z=0; z < workers_mpi.rows_z; z++) {
      for (y=0; y < workers_mpi.rows_y; y++) {
        index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
        for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
          lbm_gridinfo_w[index].fdis[k] = buffer_boundary_x_lbm[j];
          j++;
        }
        lbm_gridinfo_w[index].rho = buffer_boundary_x_lbm[j];
        j++;
        for (dim=0; dim < 3; dim++) {
          lbm_gridinfo_w[index].u[dim] = buffer_boundary_x_lbm[j];
          j++;
        }
      }
    }
  ...
  for (x=0; x < workers_mpi.rows_x; x++) {
   for (z=0; z < workers_mpi.rows_z; z++) {
     for (y=y_start; y <= y_end; y++) {
      index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
       for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
          lbm_gridinfo_w[index].fdis[k] = buffer_boundary_y_lbm[j] ;
          j++;
       }
       lbm_gridinfo_w[index].rho  = buffer_boundary_y_lbm[j] ;
       j++;
       for (dim=0; dim<3; dim++) {
         lbm_gridinfo_w[index].u[dim] = buffer_boundary_y_lbm[j] ;
         j++;
       }
      }
     j += 9*SIZE_LBM_FIELDS;
  ...
  for (x=0; x < workers_mpi.rows_x; x++) {
   for (y=0; y < workers_mpi.rows_y; y++) {
     for (z=z_start; z <= z_end; z++) {
      index = x*workers_mpi.layer_size + z*workers_mpi.rows_y + y;
       for (k=0; k < (SIZE_LBM_FIELDS-4); k++) {
         lbm_gridinfo_w[index].fdis[k] = buffer_boundary_z_lbm[j] ;
         j++;
       }
       lbm_gridinfo_w[index].rho = buffer_boundary_z_lbm[j];
       j++;
       for(dim=0; dim<3; dim++) {
         lbm_gridinfo_w[index].u[dim] = buffer_boundary_z_lbm[j];
         j++;
       }
      }
     j += 9*SIZE_LBM_FIELDS;
