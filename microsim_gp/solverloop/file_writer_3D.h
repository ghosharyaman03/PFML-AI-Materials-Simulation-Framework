...
// newly added
void write_cells_hdf5_3D_mpi_new(hid_t file_id, struct fields* gridinfo);

// writing xdmf files from main
...
void writetofile_mpi3D_hdf5(struct fields* gridinfo, char *argv[], long t)
{
  ...
  if (USE_NEW_IO_BUFF)
  {
    write_cells_hdf5_3D_mpi_new(file_id, gridinfo);
  }
  else
  {
...
//   sprintf(filename_hdf5, "DATA/%s_%ld_%ld.h5", argv[3], numworkers, t);
  sprintf(filename_hdf5, "DATA/%s_%ld.h5", argv[3], t);
  if(taskid == MASTER){
    printf("\n\n- [%d] readfromfile_mpi3D_hdf5 `%s` \n\n", taskid, filename_hdf5);
  }
    ...
    for (dim=0; dim < DIMENSION; dim++) {
      for (x=0; x < workers_mpi.rows[X]; x++) {
        for (z=0; z < workers_mpi.rows[Z]; z++) {
          for (y=0; y < workers_mpi.rows[Y]; y++) {
            index        = (x + workers_mpi.offset_x)*workers_mpi.layer_size   + (z + workers_mpi.offset_z)*workers_mpi.rows_y + (y + workers_mpi.offset_y);
            value        = lbm_gridinfo_w[index].u[dim];
            fwrite(&value, sizeof(double), 1, fp);
          }
        }
      }
...
void write_cells_hdf5_3D_mpi_new(hid_t file_id, struct fields* gridinfo_w) {
  ...
  long NUM_FIELDS = (LBM && !LBM_ALL_SAVE) ? (IO_NUM_FIELDS - (1+27)) : IO_NUM_FIELDS;
...
void write_cells_hdf5_3D_mpi(hid_t file_id, struct fields* gridinfo_w) {
  ...
  if( LBM && LBM_ALL_SAVE && DIMENSION == 3){
    size_fields += (1 + 27 );
  }
  ...
  for (x=0; x < workers_mpi.rows[X]; x++) {
    for (z=0; z < workers_mpi.rows[Z]; z++) {
      for (y=0; y < workers_mpi.rows[Y]; y++) {
        ...
        if (!ISOTHERMAL) {
          buffer[size][index_to] = gridinfo_w[index].temperature;
          size +=1 ;
        }

        if(LBM && LBM_ALL_SAVE && (DIMENSION == 3) ){
          buffer[size][index_to] = lbm_gridinfo_w[index].rho;
          size +=1 ;
          for(unsigned int a = 0 ; a < 27 ; a++){
            buffer[size + a][index_to] = lbm_gridinfo_w[index].fdis[a] ;
          }
          size += 27 ;
        }
...
void read_cells_hdf5_3D_mpi(hid_t file_id, struct fields* gridinfo_w) {
  ...
  if(taskid == MASTER){
    printf("\n - [%d] size fields = %ld ", taskid, size_fields);
  }
  ...
  for (i = 0; i < size_fields; i++) {
    ...
    if(taskid == MASTER){
      printf("\n - [%d] reading %s", taskid, coordNames[i]);
    }
  ...
  for (x=0; x < workers_mpi.rows[X]; x++) {
    for (z=0; z < workers_mpi.rows[Z]; z++) {
      for (y=0; y < workers_mpi.rows[Y]; y++) {
        ...
        if (!ISOTHERMAL) {
          gridinfo_w[index_to].temperature = buffer[size][index];
          size += 1 ;
        }

        if(LBM && DIMENSION == 3){
          lbm_gridinfo_w[index_to].rho = buffer[size][index];
          size += 1 ;
          for (unsigned int a = 0; a < 27; a++) {
            lbm_gridinfo_w[index_to].fdis[a] = buffer[size][index];
            size += 1 ;
          }
