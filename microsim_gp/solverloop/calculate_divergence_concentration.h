...
// 	c_old[k] = c_mu(gridinfo_w[center].compi, T, bulk_phase,k);
      ...
      if(LBM && (grad->bulk_phase == NUMPHASES-1)) {
        for (k=0; k<NUMCOMPONENTS-1; k++) {
          vgradc = 0.0;
          for (dim=0; dim < DIMENSION; dim++) {
            vgradc += lbm_gridinfo_w[center].u[dim]*grad->gradcomp[dim][k];
          }
          gridinfo_w[center].composition[k] -= vgradc*gridinfo_w[center].phia[NUMPHASES-1]*(deltat);
        }
      }
...
//           deltac[k] -= vgradc*gridinfo_w[center].phia[NUMPHASES-1]*(deltat);
        }
      }
      if (BINARY) {
        inv_dcdmu[0][0]              = 1.0/dcdmu[0][0];
...
//         }
      ...
      if (!(DILUTE || BINARY || TERNARY)) {
        if (USE_GSL_MATINV)
        {
          matinv_gsl_Copy_to_Mat(dcdmu_gsl_data.MAT, dcdmu);
          matinv_gsl_Invert(&dcdmu_gsl_data);
          matinv_gsl_Copy_from_Mat(inv_dcdmu, dcdmu_gsl_data.INV);
        }
        else
        {
          matinvnew(dcdmu, inv_dcdmu, NUMCOMPONENTS-1);
...
//       gridinfo_w[center].phia[b] = gridinfo_w[center].phia[b] + grad->deltaphi[b];
    ...
    for (k=0; k<NUMCOMPONENTS-1; k++) {
       ...
       //
      //  if (gridinfo_w[center].compi[k] < 0 ){
      //   printf("\n- [%d] (%ld, %ld, %ld) comp[%ld] = %le", taskid,x, gidy%workers_mpi.rows_y, gidy/workers_mpi.rows_y, k, gridinfo_w[center].compi[k] ) ;
      //  }
    }
  }
}
