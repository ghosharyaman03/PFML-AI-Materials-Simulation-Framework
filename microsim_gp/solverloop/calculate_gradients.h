...
//                                     + (iter_gridinfo_w[front].disp[Y][2]      - iter_gridinfo_w[back].disp[Y][2])/deltax); 
        ...
        grad->strain[X].xy =  (0.25)*((iter_gridinfo_w[right].disp[X][2]      - iter_gridinfo_w[left].disp[X][2]) 
                                    + (iter_gridinfo_w[front].disp[Y][2]      - iter_gridinfo_w[back].disp[Y][2])) - grad->eigen_strain[X].xy; 
...
// 
    for (gidy=0; gidy <= (workers_mpi.end[Y]+2); gidy++) {				//gidy=0  end[Y]+2
     ...
     //Calculating average stiffness
     if ((x > 2 && x <= workers_mpi.end[X]) && (gidy > 2 && gidy <= (workers_mpi.end[Y]))) {
      workers_avg_stiffness.C11 += grad->stiffness_c[X].C11;
      workers_avg_stiffness.C12 += grad->stiffness_c[X].C12;
      workers_avg_stiffness.C44 += grad->stiffness_c[X].C44;
     }
     //Calculating average stresses
     if ((x > 2 && x <= workers_mpi.end[X]) && (gidy > 2 && gidy <= (workers_mpi.end[Y]))) {
      workers_avg_stress.xx += grad->stiffness_c[X].C11*grad->strain[X].xx + grad->stiffness_c[X].C12*grad->strain[X].yy;
      workers_avg_stress.yy += grad->stiffness_c[X].C12*grad->strain[X].xx + grad->stiffness_c[X].C11*grad->strain[X].yy;
      workers_avg_stress.xy += 2.0*grad->stiffness_c[X].C44*grad->strain[X].xy;
     }
   }
}
  ...
  //For shear strains
      ...
      if ((x > 0) && (((gidy-workers_mpi.rows_y)/workers_mpi.rows_y) > 0) && (((gidy+workers_mpi.rows_y)/workers_mpi.rows_y) < (workers_mpi.rows_z)) ) {
        grad->strain[X].xz =  (0.25)*((iter_gridinfo_w[top].disp[X][2]        - iter_gridinfo_w[bottom].disp[X][2])  
                                    + (iter_gridinfo_w[front].disp[Z][2]      - iter_gridinfo_w[back].disp[Z][2])) - grad->eigen_strain[X].xz;
      ...
      if ((gidy > 0) && (((gidy-workers_mpi.rows_y)/workers_mpi.rows_y) > 0) && (((gidy+workers_mpi.rows_y)/workers_mpi.rows_y) < (workers_mpi.rows_z)) ) {
        grad->strain[X].yz =  (0.25)*((iter_gridinfo_w[right].disp[Z][2]      - iter_gridinfo_w[left].disp[Z][2])  
                                    + (iter_gridinfo_w[top].disp[Y][2]        - iter_gridinfo_w[bottom].disp[Y][2])) - grad->eigen_strain[X].yz;
...
// For stiffness
    for (gidy=1; gidy <= (workers_mpi.layer_size-1); gidy++) {				//gidy=0  end[Y]+2
     ...
     y_ = gidy%workers_mpi.rows_y;
     z_ = gidy/workers_mpi.rows_y;

      //Calculating average stiffness
     if ((x > 2 && x <= workers_mpi.end[X]) && ((y_ > 2 && (y_<= workers_mpi.end[Y]))) && ((z_>2)&&(z_<= workers_mpi.end[Z]))) {
      workers_avg_stiffness.C11 += grad->stiffness_c[X].C11;
      workers_avg_stiffness.C12 += grad->stiffness_c[X].C12;
      workers_avg_stiffness.C44 += grad->stiffness_c[X].C44;
     }
     //Calculating average stresses
     if ((x > 2 && x <= workers_mpi.end[X]) && ((y_ > 2 && (y_<= workers_mpi.end[Y]))) && ((z_>2)&&(z_<= workers_mpi.end[Z]))) {
      workers_avg_stress.xx += grad->stiffness_c[X].C11*grad->strain[X].xx + grad->stiffness_c[X].C12*(grad->strain[X].yy + grad->strain[X].zz);
      workers_avg_stress.yy += grad->stiffness_c[X].C12*(grad->strain[X].xx + grad->strain[X].zz) + grad->stiffness_c[X].C11*grad->strain[X].yy;
      workers_avg_stress.zz += grad->stiffness_c[X].C12*(grad->strain[X].xx + grad->strain[X].yy) + grad->stiffness_c[X].C11*grad->strain[X].zz;
      workers_avg_stress.xy += 2.0*grad->stiffness_c[X].C44*grad->strain[X].xy;
      workers_avg_stress.xz += 2.0*grad->stiffness_c[X].C44*grad->strain[X].xz;
      workers_avg_stress.yz += 2.0*grad->stiffness_c[X].C44*grad->strain[X].yz;
     }
   }
}
    ...
    if(LBM) {
      for (k=0; k < NUMCOMPONENTS-1; k++) {
        if ((x <=(workers_mpi.end[X]+2)) && (x > 0)) {
          grad->gradcomp[X][k]  = 0.5*(gridinfo_w[front].composition[k] - gridinfo_w[back].composition[k])/deltax;
        } else {
          grad->gradcomp[X][k] = 0.0;
        }
        if (gidy > 0) {
          grad->gradcomp[Y][k] = 0.5*(gridinfo_w[right].composition[k]  - gridinfo_w[left].composition[k])/deltay;
        } else {
          grad->gradcomp[Y][k] = 0.0;
        }
      }
    }
//     if (grad->interface) {
...
void calculate_gradients_concentration_3D(long x, struct gradlayer **gradient) {
  ...
  for (gidy=1; gidy < workers_mpi.layer_size-1; gidy++) {
    ...
    if(LBM) {
      ...
      //printf("\ngradComp(%ld,%ld,%ld)", x, gidy/workers_mpi.rows_y, gidy%workers_mpi.rows_y) ;
      //printf("\t: %ld(%ld,%ld)(%ld,%ld)(%ld,%ld)",center,right, left,top, bottom, front, back) ;
      ...
      for (k=0; k < NUMCOMPONENTS-1; k++) {
        ...
        if ( gidy % workers_mpi.rows_y > 0 && gidy % workers_mpi.rows_y < workers_mpi.end[Y]+2 ) {
          grad->gradcomp[Y][k] = 0.5*(gridinfo_w[right].composition[k]  - gridinfo_w[left].composition[k])/deltay;
        } else {
          grad->gradcomp[Y][k] = 0.0;
        }
        
        if ( gidy / workers_mpi.rows_y > 0 && gidy / workers_mpi.rows_y < workers_mpi.end[Z]+2 ) {
          grad->gradcomp[Z][k] = 0.5*(gridinfo_w[top].composition[k]  - gridinfo_w[bottom].composition[k])/deltaz;
        } else {
          grad->gradcomp[Z][k] = 0.0;
        }

        //if( fabs(grad->gradcomp[X][k]) > 1e-12 || fabs(grad->gradcomp[Y][k]) > 1e-12 || fabs(grad->gradcomp[Z][k]) > 1e-12 ){
        //  printf("\n- gradComp (%2.5f, %2.5f, %2.5f)",  grad->gradcomp[X][k], grad->gradcomp[Y][k], grad->gradcomp[Z][k]) ;
        //}
      }
    }
